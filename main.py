...
tram_station = location.Location("Трамвайная станция", {}, 0, "", underlocs={tram.name:tram})
...
the_crossroads = location.Location(
    ...
    underlocs={shop.name:shop,
        tram_station.name:tram_station
    },
)
...
commands2 = ["exit", "help", "skills", "inv", "map", "locs", "go", "quit", "talk", "fight"]
...
class Save(AllItems):
        ...
        if self.game.choice == LOADSAVE:
            ...
            while True:
                ...
                if   self.command == INV and correct_command2():  # Вещи
                    ...
                    self.arg = str(self.choice).lower().split(" ")[1]
                    if self.arg in self.total_location.ent_names:
                        if self.total_location.ent_names[self.arg].identifier == entity.EntType.BOSS:
    ...
    def show_skills(self):
        for skill_name in self.player.skills:
            console.print(f"[bold white]{self.player.skills[skill_name][0].name}[/bold white] - [blue]{skill_name}.[/blue] [yellow]{self.player.skills[skill_name][1]}[/yellow]")
    ...
    def show_locations(self):
        ...
        if (
            ...
            # Показать подлокации
            console.print(
                "[bold green]Подлокации[/bold green]"
            )
            for num, under_loc in enumerate(self.total_location.underlocs.values(), 1):  # Циклом перебераем подлокации
                console.print(
    ...
    def menu(self):
        ...
        self.choice = str(
            Input(
                ...
                in commands2+[str(ch) for ch
                        in range(1, len(self.total_location.names))],
            )
        ).lower()
